...
RapidCopy can be restarted from the web GUI. If that fails, you can restart the service from command-line:
    ...
    :::bash
    sudo service rapidcopy restart
...
This means your remote server requires that the locale matches with the RapidCopy app.
We can fix this my changing the locale for RapidCopy.
For RapidCopy docker, try adding the following options to the `docker run` command:
```
-e LC_ALL=en_US.UTF-8
-e LANG=en_US.UTF-8
```

See [this issue](https://github.com/ipsingh06/rapidcopy/issues/66) for more details.
