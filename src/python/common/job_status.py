...
# Extracted from lftp/job_status.py to support multiple transfer backends.
...
class JobStatus:
    """
    Represents the status of a single transfer job.
    Backend-agnostic: used by both lftp and rclone backends.
    """
    ...
    def __init__(self, job_id: int, job_type: Type, state: State, name: str, flags: str):
        ...
        self.__name = name
        self.__flags = flags
        self.__total_transfer_state = JobStatus.TransferState(None, None, None, None, None)
        # dict of active file transfer states, maps filename to their transfer state
        # there's no hierarchical info for now
        self.__active_files_state: dict[str, JobStatus.TransferState] = {}
    ...
    def total_transfer_state(self, total_transfer_state: TransferState):
        if self.__state == JobStatus.State.QUEUED:
