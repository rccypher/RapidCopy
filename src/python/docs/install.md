...
RapidCopy requires installation on a local machine.
...
RapidCopy supports Windows via the Docker container.
...
3. Open the Docker terminal and run the RapidCopy image with the following command:
...
4. Access application GUI to verify RapidCopy is running.
...
RapidCopy's web-based GUI can be accessed at [http://localhost:8800](http://localhost:8800).
...
1. You will need to generate a public-private key pair.
   ...
   that walks you through this process.
    ...
    !!! note
        Make sure the access is set up for the user under which RapidCopy is running.
